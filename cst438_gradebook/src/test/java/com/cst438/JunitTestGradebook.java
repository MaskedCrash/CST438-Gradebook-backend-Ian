...
public class JunitTestGradebook {
	...
	public void gradeAssignment() throws Exception {
		...
		response = mvc
				.perform(MockMvcRequestBuilders.put("/gradebook/1").accept(MediaType.APPLICATION_JSON)
						.content(asJsonString(result)).contentType(MediaType.APPLICATION_JSON)).andDo(print())
				.andReturn().getResponse();
	...
	public void addAssignment() throws Exception {
		...
		// end of mock data
		//Reminder: AssignmentDTO(int assignmentId, int courseId, String assignmentName, String dueDate, String courseTitle)
		AssignmentListDTO.AssignmentDTO assignmentDTO = new AssignmentListDTO.AssignmentDTO(assignment.getId(),assignment.getCourse().getCourse_id(),assignment.getName(),"2022-05-10",assignment.getCourse().getTitle());
		...
		//given(assignmentRepository.findById(1)).willReturn(Optional.of(ag));

		// send updates to server
		/*response = mvc
				.perform(MockMvcRequestBuilders.put("/assignment").accept(MediaType.APPLICATION_JSON)
						.content(asJsonString(result)).contentType(MediaType.APPLICATION_JSON)).andDo(print())
				.andReturn().getResponse();
		*/
	...
	public void updateAssignmentName() throws Exception {
		...
		// mock database data"/assignment/{assignmentId}/update"
		Course course = new Course();
		course.setCourse_id(TEST_COURSE_ID);
		course.setSemester(TEST_SEMESTER);
		course.setYear(TEST_YEAR);
		...
		Assignment postchange = new Assignment();
		postchange.setName("Assignment2");
		// given -- stubs for database repositories that return test data
		given(assignmentRepository.findById(1)).willReturn(Optional.of(assignment));
		...
		// then do an http get request for assignment 1
		response = mvc.perform(MockMvcRequestBuilders.put("/assignmentupdate/1").content(asJsonString("Assignment2")).accept(MediaType.APPLICATION_JSON))
				.andDo(print()).andReturn().getResponse();
		// verify return data with entry for one student without no score
		assertEquals(200, response.getStatus());
		...
		// verify that a save was NOT called on repository because student already has a
		// grade
		verify(assignmentRepository, times(1)).save(postchange);
